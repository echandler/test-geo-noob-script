...
const save = () => {
  ...
  for (let i = 1; i <= 5; i += 1) {
    ...
    if (roundTime < 0) return; // Bail out and don't save anything.
    finalTime += roundTime;
...
const resetGame = () => {
  if (isCountryStreak != null || currentRound !== 0) {
    ...
    [1,2,3,4,5].forEach((num) => setRoundTime(num,  "--:--"));
  }
};
...
const showFinalTimes = () => {
  ...
  for (let i = 1; i <= 5; i += 1) {
    const key = `${i}`;
    let time = (roundsTime[key].end - roundsTime[key].begin);
  ...
  const scoreBarLabel = document.createElement('div');//document.body.querySelector('[data-qa="result-view-top"]');
  scoreBarLabel.id = "timesTable";
  scoreBarLabel.innerHTML = `${scoreBarLabel.innerHTML}
    <div style="font-size: var(--font-size-12); color: var(--ds-color-white); margin-top: 1em;position: absolute; top: 10px; left: 10px; background: grey; padding: 1em;}">
      ...
      <center>
        <table cellpadding=5 cellspacing=0 border=1 style="font-size: var(--font-size-12); color: var(--ds-color-white); margin-top: 0.5em;">
          <tbody>
            ...
            <tr style="text-align: center;"><td>${times.join('</td><td>')}</td></tr>
          </tbody>
        </table>
      </center>
    </div>
  `;
  document.body.appendChild(scoreBarLabel);
  save();
  resetGame();
};
  ...
  if (document.querySelector('.game-layout')) {
    ...
    if (isCountryStreak) {
      ...
      for (let i = 1; i < currentRound; i += 1) {
        let time = roundsTime[i].end - roundsTime[i].begin;
        time = time < 0? 0: time;
        totalTimeMs += time;
      }
    ...
    if (currentRound >= 5 && document.querySelector('button[data-qa="play-again-button"]')) {
      showFinalTimes();
    }
  } else {
    resetGame();
    ...
    const observer = new MutationObserver(() => {
      ...
      if (gameLayout) {
        if (resultLayout) {
          ...
          document.getElementById('timesTable')?.remove();
          startRound();
        }
      } else {
          document.getElementById('timesTable')?.remove();
